...
mine.data <- read.csv2(file = "../../FeGenie/FeGenie-geneSummary_mod.csv", sep = ",",
...
mine.data<-mine.data %>%
  filter(!str_detect(category, c('iron_aquisition-siderophore_transport|iron_aquisition-siderophore_transport_potential|iron_gene_regulation')))

#Add site metadata
mine.data$Site<-mine.data$genome.assembly
mine.data<-separate(mine.data,Site,into = c("Site", "Bin"), sep = "(?=_SF)")

#add 1s and sum presence by site
col<-rep(c(1), 3453)
mine.data<-cbind(col,mine.data)

mine.data <- mine.data %>%
  group_by(Site, category) %>% #grouping taxa together for the same site
  summarise(value=sum(col))

#Separate Site into month and site
mine.data$Site<-gsub("4_1", "4.1", mine.data$Site)
mine.data$Site<-gsub("8_1", "8.1", mine.data$Site)
mine.data<-separate(mine.data,Site,into = c("Site", "Month"), sep = "(_)")


# Plot MEBS completeness
mine.heatmap <- ggplot(data = mine.data, mapping = aes(x = Month,
                                                       y = category,
                                                       fill = value)) +
  geom_tile() +
  xlab(label = "Month") +
  ...
  scale_y_discrete(limits = rev(levels(as.factor(mine.data$category))))
