...
coverm_long_proteo <- coverm_long %>% filter(grepl("p__Pseudomonadota", p))
...
coverm_long <- coverm_long %>% filter(!grepl("p__Pseudomonadota|p__Thermoproteota", p))
...
write_delim(coverm_long_final, file = "Output/coverm_long_final_08.07.25.tsv", 
...
rare_MAGs <- rare_MAGs %>%
  rename("rare_biosphere" = "unique(coverm_0.1$user_genome)")
rare_MAGs <- gtdbtk %>%
  dplyr::select("user_genome", "classification") %>%
  right_join(rare_MAGs, by = c("user_genome" = "rare_biosphere"))

write_delim(rare_MAGs, file = "Output/MAGs_RareBiosphere_08.07.25.txt", col_names = TRUE,
            delim = "\t")
...
col_vector <- c("#7FC97F","#BEAED4","#d9d9d9","#A6CEE3","#8dd3c7","#fb9a99","#BF5B17",
                ...
                "#666666","#FFFF99","#1F78B4","#B2DF8A", "#BC80BD")
...
ggsave("Output/CoverM_Abundance_Top10perSite_GTDBv2.1_biorender_trans_08.07.25.png", plot, width = 10, height = 5, dpi = 500,
       bg = "transparent")
ggsave("Output/CoverM_Abundance_Top10perSite_GTDBv2.1_biorender_trans_08.07.25.pdf", plot, width = 10, height = 5, dpi = 500,
       bg = "transparent")
...
# grab abundance data and format for the rare biosphere
coverm_long_rare <- coverm_long_final %>%
  filter(user_genome %in% rare_MAGs$user_genome) #%>%

#get MAGs mapped to their own site where they were ID'd
filtered_df_rare <- coverm_long_rare %>%
  mutate(MAG_Site = str_extract(user_genome, "^[0-9]+(?:_[0-9]+)?")) %>%
  filter(MAG_Site == as.character(Site))
filtered_df_rare$value <- as.numeric(filtered_df_rare$value)

#group by for plotting
filtered_df_rare <- filtered_df_rare %>%  
  group_by(Taxa, SiteFull, Site) %>%
  summarise(value=sum(as.numeric(value))) %>% #summing the group
  ungroup()
...
plot <- filtered_df_rare %>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5),
        ...
        #panel.border = element_blank()) + #turn this off to get the outline back)
  scale_y_continuous(expand = c(0, 0)) + #turn this on to make it look aligned with ticks
  ggtitle("Rare SFB Biosphere") + #Change for top X grabbed
  facet_grid(.~Site, scales = "free") 
#scale_y_continuous(breaks = seq(0, 45, by=5))
#coord_flip()
plot
...
ggsave("Output/CoverM_RareBiosphere_r226.png", plot, width = 10, height = 5, dpi = 500,
       bg = "transparent")
ggsave("Output/CoverM_RareBiosphere_r226.pdf", plot, width = 10, height = 5, dpi = 500,
       bg = "transparent")
